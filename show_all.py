...
def get_image_size():
	img = cv2.imread("F:/Sign-Language-master/gestures/0/100.jpg",0)
	return img.shape

gestures = os.listdir('F:\Sign-Language-master\gestures\\')
gestures.sort(key = int)
begin_index = 0
end_index = 10
image_x, image_y = get_image_size()

if len(gestures)%10 != 0:
	rows = int(len(gestures)/10)+1
else:
	rows = int(len(gestures)/10)
